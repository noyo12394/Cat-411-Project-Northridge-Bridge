      ...
      <div className="grid gap-5 lg:grid-cols-6">
        {pipeline.map((step, index) => (
          <div key={step.file} className="relative rounded-[28px] border border-white/80 bg-white/92 p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
            ...
            <h3 className="mt-3 text-lg font-semibold leading-snug tracking-[-0.03em] text-slate-950">{step.label}</h3>
            <div className="mt-4 space-y-3 text-sm text-slate-600">
              <div className="inline-flex max-w-full rounded-2xl border border-slate-200/80 bg-slate-50 px-3 py-2 font-mono text-[11px] leading-5 text-slate-700 break-all">
                {step.file}
              </div>
              <p>{step.output}</p>
            </div>
